...
const ClickSpark: React.FC<ClickSparkProps> = ({
  sparkColor, // Remove default value here
  sparkSize = 10,
  sparkRadius = 15,
  sparkCount = 8,
  duration = 400,
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      sparksRef.current = sparksRef.current.filter((spark: Spark) => {
        ...
        ctx.strokeStyle = getSparkColor(); // Use dynamic color
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
  ...
  return (
    <div className="dark:bg-gray-950 relative w-full h-full" onClick={handleClick}>
